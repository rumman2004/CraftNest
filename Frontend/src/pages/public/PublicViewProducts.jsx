...
const FullScreenWrapper = ({ children }) => (
  <div
    ...
    style={{
      background: "linear-gradient(135deg,#f3f4f7 0%,#eef0f5 50%,#f3f4f7 100%)",
    }}
  >
    {children}
  </div>
    ...
    {floatingIcons.map((p, i) => {
      ...
      return (
        <motion.div
          ...
          animate={{ y: [0, -16, 0], rotate: [0, 10, -10, 0], scale: [1, 1.12, 1] }}
          transition={{
...
const SpinnerRing = ({ size, duration, opacity, reverse = false }) => (
  <motion.div
    ...
    style={{
      width: size, height: size, margin: "auto",
      ...
      <div className="px-8 sm:px-12 py-10 sm:py-14 text-center">
        <div className="relative w-20 h-20 sm:w-24 sm:h-24 mx-auto mb-6 sm:mb-8">
          ...
          >
            ...
            >
              <PackageSearch size={18} className="text-white sm:hidden"       strokeWidth={1.8} />
              <PackageSearch size={22} className="text-white hidden sm:block"  strokeWidth={1.8} />
            </div>
          </motion.div>
        </div>
        <motion.div
          ...
          <div className="flex items-center justify-center gap-1.5">
            {[0, 1, 2, 3].map((i) => (
              <motion.div
                ...
                style={{ background: "linear-gradient(to right,#d4b26a,#13213c)" }}
              />
            ))}
          </div>
        </motion.div>
...
const ErrorScreen = ({ onRetry, onBack, isUser = false }) => {
      ...
      <GlassCard className="w-full max-w-sm sm:max-w-md">
        <div className="px-6 sm:px-8 py-10 sm:py-12 text-center">
          ...
          >
            ...
            >
              <AlertCircle size={28} className="text-white sm:hidden"       strokeWidth={1.6} />
              <AlertCircle size={34} className="text-white hidden sm:block"  strokeWidth={1.6} />
              <motion.div
                animate={{ rotate: 360 }}
          ...
          <motion.p
            ...
            animate={{ opacity: 1 }}
            transition={{ delay: 0.3 }}
            className="text-xs sm:text-sm mb-6 leading-relaxed max-w-xs mx-auto"
            style={{ color: "#4f6080" }}
          >
          ...
          >
            <button
              onClick={() =>
                navigate(isUser ? "/user/home" : "/home")
              }
              className="inline-flex items-center gap-1.5
                text-xs font-semibold transition-colors duration-200"
              style={{ color: "#4f6080" }}
              onMouseEnter={(e) => (e.currentTarget.style.color = "#d4b26a")}
...
// ── Shared detail/list view logic ──────────────────────────────────────────
const ProductViewPage = ({ view, isUser }) => {
  const { id }   = useParams();
  const navigate = useNavigate();

  const backPath  = isUser ? "/user/products" : "/shop";
  ...
  if (view === "detail" && product) {
    return (
      <AnimatePresence mode="wait">
        ...
        >
          {/* ✅ ProductDetails handles its own sticky topbar + breadcrumb
               It auto-detects /user/* vs /shop via useLocation */}
          <ProductDetails
            product={product}
...
// ✅ Both public and user views use the same component — isUser flag handles differences
export const PublicViewProducts = ({ view = "list" }) => (
  <ProductViewPage view={view} isUser={false} />
);

export const UserViewProducts = ({ view = "list" }) => (
  <ProductViewPage view={view} isUser={true} />
);

export default PublicViewProducts;